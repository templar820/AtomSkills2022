...
const AccountMenu = (props: MOBXDefaultProps & {headerRef: any}) => {
  ...
  const role = userStore.roles.find(r => r.name_role === userStore.user.role);
    ...
    <>
      ...
      >
        <div className="p-3 pb-1">
          ...
          <Typography variant="body1">{role?.caption_role}</Typography>
        </div>
        <div className="dropdown-menu header__drop-down">
          <Dropdown.Divider/>
          <Dropdown.Item
              onClick={() => {
                authService.logout();
